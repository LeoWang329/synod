/**
 * workflows/qa-loop.mjs — deepseek 出题 → minimax 回答 → deepseek 评审 的回退循环。
 *
 * 流程:
 *   1. deepseek 就给定主题出一道有明确判定标准的问题。
 *   2. backtrack 循环(最多 3 轮):
 *        produce: minimax-m3 回答(首轮答原问题,后续带 deepseek 反馈重答)。
 *        review:  deepseek 评审该回答 → PASS / FAIL(+ 改进反馈)。
 ...
 * deepseek 的出题与评审在同一个会话 history 里运行(对本 flow 可接受,甚至有益)。
 * 冷启动从 3–7 次降到 2 次:deepseek 一条 session,minimax 一条 session。
 */
...
export const meta = {
  description: "deepseek 出题 → minimax 回答 → deepseek 评审,失败带反馈重答(≤3 轮)",
};
...
export async function run(ctx, input) {
  ...
  // 2) minimax 回答 + deepseek 评审 的回退循环
