...
test("flows 段:多目录,相对路径按 config 层目录 resolve;校验非字符串拒绝", async () => {
  // 两个子用例各用独立目录:同进程内重写同一路径会命中 ESM 模块缓存。
  const a = makeDirs();
  writeFileSync(join(a.cwd, "synod.config.mjs"), `export default {
    flows: ["./flows", "./more-flows"],
  };`);
  const cfg = await loadConfig({ cwd: a.cwd, home: a.home });
  assert.deepEqual(cfg.flows, [join(a.cwd, "flows"), join(a.cwd, "more-flows")]);

  const b = makeDirs();
  writeFileSync(join(b.cwd, "synod.config.mjs"), `export default { flows: [123] };`);
  await assert.rejects(loadConfig({ cwd: b.cwd, home: b.home }), /flows.*string/i);
});
