...
### 2.5 R2 复审发现(2026-06-11,阶段 0 + 1A/1B 合并后全量复审)

> 4 路并行逐行精读(进程治理 / 后端插件化与配置 / flow API / CLI 入口),每条均经实际代码引用核实;另独立再现已知 P1-8/P1-9/P1-11,佐证其真实性。编号接续 §2.2/§2.3(P0-27、P1-28..31、P2-32..45)。修复排期见 `docs/V1.md` 看板「复审 R2」条目。

**P0-27 stdin 无 `'error'` 监听 + 死管道裸写 → EPIPE uncaughtException → 整个 synod 进程(连同所有其它会话)崩。**
- generic-cli.mjs:84 `proc.stdin.end(String(message))`,stdin 流上无任何 `'error'` 监听(`proc.on("error")` 只接 spawn 错误)。`promptVia:"stdin"` 的 CLI 在吃完 prompt 前退出(参数错 usage-exit / 启动即崩 / prompt 超 64KB 管道缓冲——flow 大 prompt 常见)→ 流 emit EPIPE → 无监听 → uncaughtException → 全局 handler 拉闸整个 REPL。已本机实证(`sh -c "exit 1"` + 1MB 写入 → `Unhandled 'error' event: write EPIPE`)。
- OmpSession 同病修了一半:stdin 只挂 `"close"`(backend.mjs:492);`request()` 裸写(:673)无守卫;`send()` 守卫只查 `exitCode !== null`(:682)**漏信号死**(SIGKILL 后 `exitCode===null`/`signalCode==="SIGKILL"`);`abort()`(:750)完全无守卫;waitIdle 每 750ms 经 state()→request() 轮询死管道。对照 CodexSession 是修过的(:911 stdin error 监听 + :1067 `#write` 门控)。
**修**:两处 stdin 挂 `'error'` 监听(记入 lastError/stderrTail,让 turn 以失败收口而非崩宿主);OmpSession.request 按 CodexSession `#write` 同款门控(`!stdin || destroyed || exitCode!==null || signalCode!==null`)同步拒绝;send 守卫补 signalCode。

**P1-28 真实 TTY 下 Ctrl-C 不产生 SIGINT → interactiveSigint 退出矩阵在主要使用场景是死代码。**
createRepl 以 `terminal: stdin.isTTY`(cli.mjs:168)进 raw mode,Ctrl-C 由 readline 接管:rl 上无 `SIGINT` 监听器时直接 `rl.close()`(PTY 实测 `process.on("SIGINT")` 不触发)。后果:第一次 Ctrl-C 实走 onClose → `await _pendingFlows` + drainAll = **静默等 in-flight turn 自然完成**(默认 30 分钟),不 abort、无反馈;"Interrupted. Cleaning up..." 永不打印。管道模式(terminal:false)不受影响,恰好让现有测试全绿。
**修**:`rl.on("SIGINT", ...)` 转发统一退出矩阵(如 `process.kill(process.pid, "SIGINT")`);与 §4.7-3 取消语义、§4.8 InputRouter 同域(1C-a)。

**P1-29 并发 `/flow` 互踩模块级 currentRuntime 单例 → 第二个 flow 崩 / 复用会话泄漏。**
cli.mjs:378-385 runFlow 无并发互斥(`_pendingFlows` 只用于退出排水),REPL 不阻塞输入,flow1 未完时可再 `/flow`。时序:flow1 set R1 → flow2 set R2(prev=R1)→ flow1 先结束 restore null → flow2 全部 `synod/flow` 原语抛 "No active flow runtime";反序则 flow1 的 reuse 会话落进 R2 的池,flow1 的 disposeRun 清不到 → 真实子进程泄漏到进程退出。
**修**:1C-a 并发治理(current-run 改 AsyncLocalStorage,§4.7-2)根治;过渡可先拒绝并发 `/flow`。

**P1-30 已退出 proc 的陈旧 pid 被无门控组杀(PID 复用误杀)。**
backend.mjs:844/:1507 close() 先行的 `terminateProcessTree(this.proc?.pid, ...)`、shutdown.mjs:36-52 closeAllLiveSessionsSync 的 pid 收集 + 幸存者 SIGKILL,均不查 `proc.exitCode/signalCode`;`kill(pid,0)` 查活不证身份。配合 P2-32(死会话滞留 `_live`)成链:长寿 REPL 中 backend 自退 → 数小时后 OS 回收 pid → 用户 close 或进程退出 → 对无辜进程(组)发 SIGTERM/SIGKILL。scheduleForceKill 修过同类(:374 有门控),这两处漏了。
**修**:两处补 `exitCode===null && signalCode===null` 门控。

**P1-31 win32 `.cmd` shim 下 generic-cli 的 prompt/model 注入 cmd.exe(BatBadBut 同型命令注入)。**
spawnPlan 在 win32 把全部 args 经 `cmd.exe /d /s /c` 转发(backend.mjs:178-185),Node 只做 MSVCRT 引号、**不转义 cmd 元字符**;generic-cli `promptVia:"arg"` 把 prompt 原文进 argv、model 不过 sanitizeAgentArg(generic-cli.mjs:59-64)。`x&calc` 这类无空格 token 被 cmd 二次解析执行。攻击面:relay 转发的其它 agent 输出、flow 拼接的文件内容。内置 backend 靠 stdin RPC + sanitizeAgentArg 防住,generic-cli 两道都缺。
**修**:win32 落到 cmd.exe 包装时 `promptVia:"arg"` 直接拒绝(或强制走 stdin);model 过 sanitizeAgentArg。

| # | 问题 | 定位 | 说明/修法 |
|---|---|---|---|
| P2-32 | omp/codex 的 proc `"close"` handler 不 untrackSession/removePidRecord → 后端自退后死会话滞留 `_live`(P1-30 使能条件)、磁盘记录 owner 活着永远收不掉 | backend.mjs:526-555,944-970 | handler 末尾补 `untrackSession(this); removePidRecord(this.id)` |
| P2-33 | spawn 成功 → trackSession/writePidRecord 之间(OMP ready 等待最长 60s)无任何登记,窗口内信号/崩溃 → detached 子进程成不可收割孤儿 | backend.mjs:479,1545-1549 | spawn 后立即登记,open 失败回滚 |
| P2-34 | reapOrphans 不复核 `pid > 1`;`pid:0` 损坏记录 → `kill(0/-0,...)` 对收尸进程**自身进程组**发信号(isAlive(0) 恒真,三重门控可凑齐) | pid-registry.mjs:107-142 | 收尸侧补 `rec.pid > 1`(与写入侧 :58 对称) |
| P2-35 | doctor() 不 await:async 的 `type:"module"` 自定义 adapter 返回 Promise → `entry.available === undefined` → 永远误报"不可用"且文案误导 | backend.mjs:1603-1609;session-manager.mjs:57 | await 各 adapter doctor(或 registerBackend 时拒 Promise 返回) |
| P2-36 | config 文件 `import` 抛的 SyntaxError 不带文件路径(只在 stack 里),cli 只打 message → 分不清 `~/.synod/` 还是项目层 | config.mjs:60-66;cli.mjs:315 | loadLayer 包 try/catch 重抛 `config error: ${msg} (in ${file})` |
| P2-37 | `/open` 护栏 check-then-act:guard 读 `_sessions.size` 与 set(open 成功后)之间隔着秒级 backend 启动,跨会话并发 fence 同时过闸 → 可越 maxSessions | repl-dispatch.mjs:101;session-manager.mjs:160 | guard 通过后同步占位(先 set 占 label 再异步 open,失败回收) |
| P2-38 | generic-cli 静默丢弃 systemPrompt/mesh:profile 的 role 在 cli backend 上无声失效(而 reuse key 仍按 systemPrompt 区分);且 `write:false` 对 type:cli 无任何实质约束 → agent-fence 的 allowWrite 护栏对 cli backend 是空保证 | backends/generic-cli.mjs:23-42 | open 收到 systemPrompt/mesh 显式报错或 stderr 警告;护栏/文档把 type:cli 视同 write 后端 |
| P2-39 | `type:"cli"` 的相对 `bin` 不按配置层目录 resolve(`type:"module"` 的 path 做了):doctor(spawnSync 无 cwd)与每 turn spawn(会话 cwd)各按不同基准解析,可探测通过、实跑 ENOENT | config.mjs:80-82 | bin 含路径分隔符时 `path.resolve(dir, b.bin)` |
| P2-40 | defer 的 suppressed 错误附着**恒不生效**:`Error.cause !== undefined` 是对构造函数取静态属性,恒假 → fn 与 defer 同时抛错时 defer 错误被静默丢弃,违背模块头注释契约 | flow/defer.mjs:85 | 删恒假检测,直接 `fnErr.suppressed = deferErr` |
| P2-41 | disposeRun 的 `entry.session.close()` 未防抛:一个会话 close 抛错 → 其余池中会话被 clear 却没关(泄漏),且异常穿透 runner 的 finally **掩盖 flow 本身的结果/原始错误**(module 型自定义 backend close 可抛;内置已 try/catch 不触发) | flow/runtime.mjs:178-190;flow/runner.mjs:22-27 | 逐条 `try { close() } catch {}`(与 logSession 同 best-effort 语义) |
| P2-42 | reviseWithHuman 静默丢弃 `profile`(及 effort/write/systemPrompt),与 agent/agentLoop 已接 resolveOpts 不一致 → 错 agent 错人设零报错 | flow/api/reviseWithHuman.mjs:86-115 | opts 原样透传内部 agent()(它自己跑 resolveOpts) |
| P2-43 | standalone flow **正常**退出路径无活会话兜底:`main().then(code => process.exit(...))` 不调 closeAllLiveSessionsSync(只在 Fatal catch 分支),fire-and-forget 的非 reuse `agent()` 子进程被 process.exit 砍成孤儿 | flow.mjs:347-353 | then 退出前调一次 closeAllLiveSessionsSync(幂等,常态 no-op) |
| P2-44 | `--task` 非交互模式被 SIGINT 打断却 `exit(0)`(interactiveSigint 语义误用于全部模式)→ 脚本/CI 无法区分成功与被中断 | cli.mjs:470;shutdown.mjs:106,116 | 仅交互 REPL 分支用 interactiveSigint,--task 路径 exit(130) |
| P2-45 | REPL `/flow <name> <input>` 把 input 原样塞 argv,parseFlowArgs 对每个 token 做 flag 匹配:input 为 `--list`/`--workflows` 等时被吞成 flag(后者还会改写 flow 发现根目录),注释声称的 "input verbatim" 不成立 | repl-dispatch.mjs:337;flow.mjs:54-91 | parseFlowArgs 支持 `--` 分隔符,REPL 侧用 `["--progress","--",name,input]` |
...
| P2 | 13–26 | 见 §2.3 表 | — |
| P0 | 27 | stdin 无 error 监听 + 死管道裸写 → EPIPE 崩整个 synod | generic-cli.mjs:84; backend.mjs:673,750 | R2;1C-a Part A |
| P1 | 28 | 真 TTY 下 Ctrl-C 不产生 SIGINT,interactiveSigint 矩阵成死代码 | cli.mjs:168; shutdown.mjs:103-117 | R2;1C-a(InputRouter/取消) |
| P1 | 29 | 并发 /flow 互踩 currentRuntime 单例 → 崩/会话泄漏 | cli.mjs:378; flow/current-run.mjs:14 | R2;1C-a(AsyncLocalStorage) |
| P1 | 30 | 已退出 proc 的陈旧 pid 无门控组杀(PID 复用误杀) | backend.mjs:844,1507; shutdown.mjs:36-52 | R2;1C-a Part A |
| P1 | 31 | win32 .cmd shim 下 generic-cli prompt/model 注入 cmd.exe | generic-cli.mjs:59-64; backend.mjs:178-185 | R2;1C-a Part A |
| P2 | 32–45 | 见 §2.5 表 | — | R2;排期见 V1.md「复审 R2」 |
