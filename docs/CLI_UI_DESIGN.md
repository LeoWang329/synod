...
> **2026-06-13 更新(原则修订,务必先读):** 本文档描述的是**已落地的 v1/v2 行式 REPL UI**。其后我们决定另起一套 **OpenCode 风格全屏 TUI**(右侧 agent 栏 + 焦点区 + 流式工具调用卡片 + 鼠标点击),它**有意推翻**下面 §0 的两条前提——「零依赖/不引 ink」(§0.1)和「一切可回读/拒绝 alt-screen」(§0.3):新 TUI **改用 [Ink](https://github.com/vadimdemedes/ink)**(覆盖不到处手写 ANSI 补)并接管全屏。另外正本清源:§0.1 的「零依赖」原指**核心层不引第三方 npm 包**(实现选择),**不等于**「synod 自包含、不复用 agent-bridge 组件」——这两件事是分开的,UI 前端层也不受核心零三方包的限制。新 TUI 的设计见其专属 spec。

---

## 0. 设计原则

1. **零依赖(仅限本文档的 v1 行式 UI;新全屏 TUI 已解禁、改用 Ink,见上方 2026-06-13 更新)**:v1 只用 ANSI 转义 + `node:readline`,不引 ink/blessed/chalk、颜色函数自写(~20 行)。此处「零依赖」= 核心不引第三方 npm 包(实现选择),≠「自包含/不复用 agent-bridge 组件」。
