...
> ⚠️ **2026-06-10 起新主线 = V1 架构重构(正式版)**:开发总入口与进度看板在 [`docs/V1.md`](V1.md)——接手开发**先读它**,按它的看板顺序执行。本文转为历史背景与三方协作工法参考(工法部分仍有效)。
>
> 🛑 **本文以下的 TL;DR / 全景 / 文档地图停在 2026-06-07,已严重过时(那时还"两条分支未推送 GitHub")——勿当现状读。** 最新进度(已交付到**阶段 1D**、阶段 0/1A-1B/1C-a/1C-b/1D 全部合并 main + 推 origin、**下一步 = 撰写阶段 2 计划**)看 [`docs/V1.md`](V1.md) 顶部「📍 当前进度快照」。本文仅供**三方协作工法 / agent-bridge 派活坑**参考。
>
