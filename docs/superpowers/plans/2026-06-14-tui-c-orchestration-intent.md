...
> **`feedbackSent` 语义(明确,避免误读)**:它是**最近一 turn**是否回喂成功(`sm.enqueue !== false`),**不是**「所有累积命令都已回喂」。每条命令的真实结果在累积明细的 `cmd → result` 里(展开 C 可见),那才是逐条真相;摘要的「· 回喂已发」只表示最近一轮编排的结果已送回发起 agent。本期保持单 bool(YAGNI);若日后要精确到「每轮回喂状态」再单列(`feedbackSentCount` 等)。
...
Expected: 三新用例 FAIL(无 Ctrl-G/Ctrl-T 处理,明细不展开/不收起)。原用例仍 PASS。
...
> 注:`expandC` 从闭包读(与同文件既有 `selIdx` 同一模式——Ink 每 render 重订阅 useInput,跨 render 读到最新值;Step1 的「连按两次收起」用例覆盖此跨-render toggle)。**唯一边界**:同一 render 内极快连按可能读旧值欠一步,与既有 selIdx 行为一致、属可接受小瑕疵(codex 已确认)。Ctrl-G=`\x07`、Ctrl-T=`\x14`(已验在 ink-testing-library 下解析为 `key.ctrl + 'g'/'t'`),P1 未占用。
...
## Task 5: 扩 smoke 验 UI 链路(appendFence→C→键盘)

**Files:**
- Modify: `scripts/smoke-tui.mjs`

> **覆盖范围(诚实标注)**:本 smoke 验的是 **UI 链路**——`store.appendFence` → 真 startTui/FocusPane 渲染 → Ctrl-G/Ctrl-T 键盘。它**不验** `control-wire → cli onFence → store.appendFence` 这条真实接线(smoke 直接调 appendFence)。那条线由 **Task1 control-wire 单测**(onFence 产出正确)+ **Task3 cli 一行静态接线**(codex 静态审 + gate)共同覆盖。
